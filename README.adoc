...
Sometimes it's not feasible to map a suboject entirely by just adding annotations.

E.g. a 'json' payload can be pointed to, but automatic unmapping of this is not feasible.
...
Mapper mapper = MAPPER.withLeafMapper((mapper, field, value) -> {
  return <new object of type destiatnionType mapped from incoming o>
}
